__kernel void bruteforce(
  ...
  ulong mnemonic_0 = mnemonic_start[0] + idx;
  ulong mnemonic_1 = mnemonic_start[1];
  ulong mnemonic_2 = mnemonic_start[2];
  ulong mnemonic_3 = mnemonic_start[3];
  ...
  bool found_target = 1;
  for(int i=0; i<target_pkhash_len; i++) {
    if(hash160_address[i] != target_pkhash[i]){
      found_target = 0;
    }
  }
